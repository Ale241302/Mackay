...
@section('content')
    <div class="container">
        <h1 class="flex-grow-1 text-center mb-0">Actividades</h1>
        <br />
        @if (in_array(22, $permisosUsuario))
            <!-- Habilita el botón de crear solo si el usuario tiene el permiso correspondiente -->
            <div class="d-flex justify-content-end">
                <a href="{{ route('tipoactividad.create') }}" class="btn btn-primary"
                    style="background-color: #1814F3; border-color: #1814F3;">+ Nueva Actividad</a>
            </div>
        @endif
        <br />
        <form id="search-form" action="{{ route('tipoactividad.index') }}" method="GET" class="mb-3">
            <br />
                    ...
                    @foreach ($userGroups as $userGroup)
                        <tr>
                            @if (in_array(21, $permisosUsuario))
                                ...
                                <td style="text-align: center">{{ $userGroup->tipo }}</td>
                                
                            @endif
                            <td class="action-buttons d-flex justify-content-center align-items-center gap-2" style="text-align: center">
                                <!-- Centra el contenido de la celda -->
                                <div class="d-flex justify-content-center align-items-center gap-2">

                                    @if (in_array(23, $permisosUsuario))
                                        <!-- Botón de editar -->
                                        <a href="{{ route('tipoactividad.edit', $userGroup->id) }}"
                                    ...
                                    @if (in_array(24, $permisosUsuario))
                                        ...
                                        @else
                                            ...
                                            <button type="button" class="btn btn-danger"
                                                ...
                                                data-action="{{ route('tipoactividad.destroy', $userGroup->id) }}">
