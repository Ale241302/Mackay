...
@section('content')
    <div class="container">
        <h1 class="flex-grow-1 text-center mb-0">Tribunal</h1>
        <br />
        @if (in_array(50, $permisosUsuario))
            <!-- Habilita el botón de crear solo si el usuario tiene el permiso correspondiente -->
            <div class="d-flex justify-content-end">
                <a href="{{ route('tribunal.create') }}" class="btn btn-primary"
                    style="background-color: #1814F3; border-color: #1814F3;">+ Nuevo Tribunal</a>
            </div>
        @endif
        <br />
        <form id="search-form" action="{{ route('tribunal.index') }}" method="GET" class="mb-3">
            <br />
                    ...
                    @foreach ($userGroups as $userGroup)
                        <tr>
                            @if (in_array(49, $permisosUsuario))
                                ...
                                <td style="text-align: center">{{ $userGroup->Ciudad->nombre }}</td>
                            @endif
                            <td class="action-buttons d-flex justify-content-center align-items-center gap-2">
                                <!-- Centra el contenido de la celda -->
                                <div class="d-flex justify-content-center align-items-center gap-2">

                                    @if (in_array(51, $permisosUsuario))
                                        <!-- Botón de editar -->
                                        <a href="{{ route('tribunal.edit', $userGroup->id) }}" class="btn btn-warning me-2"
                                    ...
                                    @if (in_array(52, $permisosUsuario))
                                        ...
                                        @else
                                            ...
                                            <button type="button" class="btn btn-danger"
                                                ...
                                                data-action="{{ route('tribunal.destroy', $userGroup->id) }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            $('#search-input').on('input', function() {
                var search = $(this).val(); // Obtiene el valor del input de búsqueda
                $.ajax({
                    url: '{{ route('tribunal.index') }}', // Ruta al controlador
                    method: 'GET', // Método de solicitud
                    data: {
                        search: search // Datos que se envían con la solicitud
                    },
                    success: function(response) {
                        $('tbody').html($(response).find('tbody')
                            .html()
                        ); // Reemplaza el contenido del tbody con los resultados obtenidos
                    },
