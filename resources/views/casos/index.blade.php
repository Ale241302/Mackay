...
@section('content')
    <div class="container">
        <h1 class="flex-grow-1 text-center mb-0">Casos</h1>
        <br />
        @if (in_array(30, $permisosUsuario))
            <div class="d-flex justify-content-end">
                <a href="{{ route('casos.create') }}" class="btn btn-primary"
                    style="background-color: #1814F3; border-color: #1814F3;">+ Nuevo Caso</a>
            </div>
        @endif
        <br />
        <form id="search-form" action="{{ route('casos.index') }}" method="GET" class="mb-3">
            ...
            <table class="table table-bordered" style="text-align: center; white-space: nowrap;">
                <thead>
                    <tr>
                        ...
                        <th style="text-align: center; white-space: nowrap;">Etapa Procesal</th>
                        <th style="text-align: center; white-space: nowrap;">Tribunal</th>
                        <th style="text-align: center; white-space: nowrap;">Sub-Casos</th>
                    ...
                    @foreach ($clientes as $cliente)
                        <tr>
                            @if (in_array(29, $permisosUsuario))
                                <td style="white-space: nowrap; text-align: center;">{{ $cliente->id }}</td>
                                <td style="white-space: nowrap;">
                                    {{ $cliente2[$cliente->empresa]->empresa ?? 'No disponible' }}
                                </td>
                                <td
                                ...
                                <td style="white-space: nowrap;  text-align: center;">
                                    {{ $procesal[$cliente->etapa_procesal]->nombre ?? 'No disponible' }}</td>
                                <td style="white-space: nowrap; text-align: center;">
                                    @php
                                ...
                                <td style="white-space: nowrap; text-align: center;">{{ $cliente->subcasos_count }}</td>
                                <td style="white-space: nowrap;">
                            ...
                            <td class="action-buttons" style="white-space: nowrap;">
                                <div class="d-inline-flex">
                                    @if (in_array(29, $permisosUsuario))
                                        <a href="{{ route('casos.ver', $cliente->id) }}" class="btn btn-warning me-2"
                                    ...
                                    @if (in_array(31, $permisosUsuario))
                                        <a href="{{ route('casos.edit', $cliente->id) }}" class="btn btn-warning me-2"
                                    ...
                                    @if (in_array(32, $permisosUsuario))
                                        <button type="button" class="btn btn-danger"
                                            ...
                                            data-action="{{ route('casos.destroy', $cliente->id) }}">
